...
def load_or_create_index(path_to_index, model):
    ...
    if os.path.exists(path_to_index):
        ...
        # 新しいインデックスを作成（モデルからベクトルの次元を取得して使 用）
...
# 新しい文書の追加（サンプル）
new_sentences = ["この方法は非常に有用だ。", "この技術はテキスト分析において大変役立つ。", "別のトピックについて話す。", "この技術は、大量のテキストデータを扱う際に特に有用で、文の意味を迅速に理解し、関連する情報を見つけ出すのに役立つ。"]
add_new_documents(new_sentences, model, index)

# クエリに基づいて検索を行い、対応する原文を取得する関数
def search_and_retrieve_original_text(query, model, index, k=3):
    # クエリをベクトル化
